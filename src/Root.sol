...
contract Root is Base, IRoot {
    ...
    function _spawn(uint8 n) private returns (address) {
        TvmCell c = tvm.insertPubkey(_images[n].si, tvm.pubkey());
        address addr;
    ...
    function deploySystem() external {
        ...
        Base(console).initMember{value: PROCESS}(CONSOLE_ID, console, eventLog, root, medium, logLevel);
        Base(eventLog).initMember{value: PROCESS}(EVENT_LOG_ID, console, eventLog, root, medium, logLevel);
        Base(medium).initMember{value: PROCESS}(MEDIUM_ID, console, eventLog, root, medium, logLevel);
    }
    ...
    function deployTokenWalletsWithKeys(uint[] keys) external override accept returns (uint16[] ids, address[] addrs) {
        for (uint key: keys) {
    ...
    function onTokenWalletDeploy(uint16 id, uint k, uint64 balance) external override {
        ...
        Base(from).initMember{value: COMPUTE}(id, _console, _eventLog, _root, _medium, _logLevel);
    ...
    function setEnv(address console, address eventLog, address root, address medium, uint16 logLevel) external override accept {
        _setEnv(console, eventLog, root, medium, logLevel);
    ...
    function updateWalletsEnv() external override adm {
        for ((address addr, ): _roster) {
            Base(addr).updateEnv{value: COMPUTE}(_console, _eventLog, _root, _medium, _logLevel);
        }
    }
    ...
    function registerOwners() external override adm {
        for ((uint8 id, OwnerInfo oi): _owners) {
            ...
            OwnerWallet(oi.addr).updateEnv{value: PROCESS}(_console, _eventLog, _root, _medium, _logLevel);
            TokenWallet(oi.tokenWalletAddr).updateEnv{value: PROCESS}(_console, _eventLog, _root, _medium, _logLevel);
        }
    }

    function updateSystemImage(CTImage console, CTImage eventLog, CTImage root, CTImage medium) external {
        _images[1] = console;
        _images[2] = eventLog;
        _images[3] = root;
        _images[4] = medium;
